...
voxel. For balloon inflation the two tangential eigenvalues are
equal ($\mu_{\theta\theta} = \mu_{\phi\phi}$), and Ogden gives
$\mu_{rr} = 2 W_1(\lambda_\theta)\,\lambda_\theta^{-4}$
(from $\lambda_r = \lambda_\theta^{-2}$). The compact Cartesian
tensor is then
...
where $\hat r(\vect x) = (\vect x - \vect x_{\text{cavity}})/r$ is
the Cartesian components of the local radial unit vector. This gives
a \textbf{fully populated symmetric $3 \times 3$ tensor field}: six
independent components varying voxel-by-voxel.
