...
complex $G(x)$ to validation RL$^{2}=0.043$ after 25 epochs
...
for a 71k-parameter 1D Fourier neural operator trained to map noisy
$\hat u(x)$ at SNR\,=\,25\,dB to the complex shear modulus $G(x)$ on
samples drawn from the brain-scale parameter prior. (b) Four held-out
samples: ground-truth and predicted real (top row) and imaginary
(bottom row) parts of $G(x)$; the FNO recovers both the smooth ramp and
the loss component.

\textbf{Figure 3. TSM-FNO architecture.}
...
\textbf{Figure 4. Acoustoelastic phantom physics.}
...
\textbf{Figure 5. Phase-A phantom validation.}
...
\textbf{Figure 6. Synthetic cohort performance.}
