...
= \sum_{j_\ax = 0}^{2}
    \frac{1}{2\Delta x}
    \bigl(\sigma_{c j_\ax}^{[i,j,k] + e_{j_\ax}}
        - \sigma_{c j_\ax}^{[i,j,k] - e_{j_\ax}}\bigr),
\end{equation}
...
(\SI{19.2}{\centi\metre} cube), followed by the curl→filter→DI→MIP
...
Phantom & GT $\mu_{\theta\theta}$ & $\mu_{\mathrm{conv}}$
        ...
        & [\si{\kilo\pascal}]     & — \\
\midrule
P1 & 4.93 & 2.44 & 3.23 & \textbf{1.32} \\
P2 & 6.12 & 2.66 & 3.48 & \textbf{1.31} \\
\midrule
...
The recovered TSM/conv ratio of $\sim 1.30$ sits below Yin's measured
$1.57$. Two known contributions to that gap, both fixable but not
addressed here (see \S\ref{sec:openissues}):
...
Phantom & GT $\mu_{\theta\theta}$ [\si{\kilo\pascal}]
        ...
        & TSM/conv & TSM/GT \\
\midrule
P1 (neo-Hookean)     & 4.93 & 2.44 & 3.23 & 1.32 & 0.65 \\
P2 (matrix + fiber)  & 6.12 & 2.66 & 3.48 & 1.31 & 0.57 \\
\midrule
Yin measured (P1)    & --- & --- & $\sim$4.4 & 1.57 & --- \\
\bottomrule
\end{tabular}
