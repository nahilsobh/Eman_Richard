...
At $N=48$ the pipeline reproduces Yin's TSM/conv ratio $= 1.57$
essentially exactly (\S\ref{sec:ring}), but the \emph{absolute}
$\mu_{\mathrm{TSM}}$ is about half of Yin's $\sim\!4.4$~kPa. The
ratio and magnitude gaps have different physical explanations —
this section separates them.

\paragraph{Ratio (TSM/conv) — closed.}
Two implementation choices moved the ratio from an early
$1.31$--$1.32$ up to Yin's $1.57$:
\begin{itemize}
...
\item Refining to $N=48$ ($\Delta x = 3.75$~mm, at the Yin voxel
      scale) — the dominant lift. The $\lambda/\Delta x\approx 9$
      resolution captures the sharp $\mu_{ij}(\vect x)$ gradient
      across the perilesional shell.
\end{itemize}
Together these bring both phantoms to TSM/conv $\in\{1.572, 1.579\}$
— matching Yin's $1.57$ signature.

\paragraph{Magnitude — open.}
$\mu_{\mathrm{TSM}} = 1.83$~kPa (P1) and $1.87$~kPa (P2) at $N=48$,
versus Yin's $\sim\!4.4$~kPa. The following is the current best
understanding of where the factor-of-two comes from.

The material tensor $\mu_{ij}(\vect x)$ we plug into the solver
\emph{already includes} the incompressible-neo-Hookean acoustoelastic
response — Ogden's small-on-large formula
$\mu_{\theta\theta} = 2W_1\lambda_\theta^{2}$ and
$\mu_{rr} = 2W_1\lambda_\theta^{-4}$ bundles the base modulus and
the geometric pre-stress contribution into a single principal-frame
wave modulus. So this is \textbf{not} a "missing acoustoelastic
term" problem for our chosen constitutive laws.

The residual gap is a \textbf{rank-2 vs rank-4 projection loss}. The
full effective elasticity tensor $C^{\text{eff}}_{ijkl}(\vect x)$ at
a pre-stressed voxel has more independent shear moduli than the two
principal eigenvalues $(\mu_{rr},\mu_{\theta\theta})$ we extract. A
wave propagating in a non-principal direction feels an effective
modulus that our rank-2 $\mu_{ij}$ can only approximate as a
mixture. Concretely, our constitutive law
\begin{equation}
\sigma_{ij} = \lambda\delta_{ij}\varepsilon_{kk}
           + \mu_{ik}\varepsilon_{kj} + \mu_{jk}\varepsilon_{ki}
\end{equation}
coincides with full isotropic elasticity when
$\mu_{ij}=\mu\delta_{ij}$, but when $\mu_{ij}$ has directional
structure it is a specific rank-2 realisation of an anisotropic
stress--strain map, not the general rank-4 $C^{\text{eff}}_{ijkl}$.
Direct inversion of curl$\,\vect u$ recovers a wave modulus averaged
over the directional content of the local field, and that average
comes out systematically lower than the principal $\mu_{\theta\theta}$
that Yin's phantom data seems to reveal.

\textbf{Follow-up.} Upgrade the constitutive core of
\texttt{navier\_solve\_3d\_tensor\_mu} from the rank-2 $\mu_{ij}$
form to a full rank-4 $C^{\text{eff}}_{ijkl}(\vect x)$ field. For
incompressible neo-Hookean, $C^{\text{eff}}_{ijkl}$ has a closed
form in terms of the principal stretches $\lambda_i(\vect x)$ and
the pre-stress $\sigma^{\text{pre}}$ (Ogden \S6.1); no new material
constants needed. Third-order Murnaghan coefficients
$(l, m, n)$ would be a further refinement on top, but those are not
measured for the Sobh--Ehman phantoms and are expected to give
smaller corrections than the rank-2$\to$rank-4 upgrade.
