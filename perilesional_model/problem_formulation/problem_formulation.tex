...
\midrule
$i = 0$ \hfill (top)      & $\sigma_{ij} n_j = 0$ ($\vect n = -\hat z$) & ghost-node stencil (eq.~\ref{eq:ghost}) \\
$i = N-1$ \hfill (bottom) & Dirichlet from driver       & set $u$ at all disk nodes \\
$j = 0$, $j = N-1$        & $u = 0$                     & Dirichlet \\
$k = 0$, $k = N-1$        & $u = 0$                     & Dirichlet \\
Bottom disk + 4 side disks & Dirichlet on face-normal $u_c$ & multi-face broadband driver \\
...
\qquad
\bar\mu_{\mathrm{top}} \;=\; \tfrac{1}{3}\operatorname{tr}\bigl(\mu(\vect x_{\mathrm{top}})\bigr).
\end{equation}
...
path. If PARDISO is unavailable we fall back to scipy's SuperLU
single-thread \texttt{spsolve}.
...
DOFs.  \textbf{PARDISO LU on 32 threads:} $\sim \SI{25}{\giga\byte}$
memory, $\sim 200$ s wall time per phantom
(bottom-face-only driver: $\sim 130$ s; multi-face broadband adds
$\sim 70$ s for the extra RHS work). SuperLU fallback runs the same
job in $\sim 30$ minutes single-threaded.
\end{grayb}
